...
Software description: Pseudo validation utility for Tota11ylost
*/
...
// Level 1: Short/ambiguous words — require word-boundary matching
...
// Level 2: Long/specific words — substring match (always blocked)
const PROFANITY_WORDS = [
  ...
  // === French - Additional insults (from french-badwords-list) ===
  'bite', 'b1te', 'b!te',
  'andouille', '@ndouille',
  'avorton', '@vorton',
  'biatch', 'b!atch',
...
function validatePseudo(pseudo) {
  ...
  // Level 1: Boundary words — only block if the word appears as a standalone segment
  // A word is "standalone" if it is NOT embedded inside a larger alphabetic word.
  // It must have a non-letter (or string boundary) on BOTH sides.
  // e.g. "massy" → 'm' + ass + 'y' → letters on both sides → OK
  // e.g. "dickinson" → dick + 'i' → letter after → OK
  // e.g. "ass123" → start + ass + '1' → non-letter on both sides → BLOCKED
  // e.g. "xxx" → start + xxx + end → non-letter on both sides → BLOCKED
  for (const word of BOUNDARY_WORDS) {
    ...
    if (index !== -1) {
      ...
      // Block only if NEITHER side has a letter (word is isolated / between non-alpha chars)
 ...
 * @param {string} errorCode - The error code or null
 * @param {HTMLElement} errorContainer - The DOM element to display error in
 */
function displayPseudoError(errorCode, errorContainer) {
